...
import { CARD_FACE_PACK_MANIFEST, CARD_BACK_PACK_MANIFEST } from "../apps/client/src/assets/cards/packManifest";
...
const BACK_IMAGE_FILENAME = "back.png";
...
const outputFile = path.resolve(clientDir, "src/assets/cards/generated/cardPackRegistry.ts");
...
function ensureBackManifestIntegrity() {
  const ids = CARD_BACK_PACK_MANIFEST.map((pack) => pack.id);
  const duplicateIds = ids.filter((id, index) => ids.indexOf(id) !== index);
  if (duplicateIds.length > 0) {
    const uniqueDuplicateIds = [...new Set(duplicateIds)];
    throw new Error(`Duplicate card-back pack ids in manifest: ${uniqueDuplicateIds.join(", ")}`);
  }
}
...
async function validateLocalBackPacks() {
  const problems: string[] = [];

  for (const pack of CARD_BACK_PACK_MANIFEST) {
    if (pack.source.type !== "local") continue;

    const folderPath = path.resolve(clientAssetsCardsDir, pack.source.folder);
    let hasBackPng = false;
    try {
      const dirEntries = await fs.readdir(folderPath, { withFileTypes: true });
      const files = dirEntries.filter((entry) => entry.isFile()).map((entry) => entry.name);
      hasBackPng = files.includes(BACK_IMAGE_FILENAME);
    } catch {
      problems.push(`Card-back pack "${pack.id}" folder not found: ${folderPath}`);
      continue;
    }

    if (!hasBackPng) {
      problems.push(`Card-back pack "${pack.id}" in folder "${pack.source.folder}" missing ${BACK_IMAGE_FILENAME}.`);
    }
  }

  if (problems.length > 0) {
    throw new Error(`Card-back pack validation failed:\n${problems.join("\n\n")}`);
  }
}
...
function generateRegistrySource() {
  ...
  const localBackPacks = CARD_BACK_PACK_MANIFEST.filter((pack) => pack.source.type === "local");
  const backPackIds = localBackPacks.map((pack) => pack.id);
  const cardBackPackIdType = toGeneratedTypeUnion(backPackIds);

  const testBackEntries =
    localBackPacks.length === 0
      ? ""
      : localBackPacks
          .map((pack) => `  ${JSON.stringify(pack.id)}: "back" as unknown as ImageSourcePropType,`)
          .join("\n");

  const runtimeBackEntries =
    localBackPacks.length === 0
      ? ""
      : localBackPacks
          .map((pack) => {
            const assetRelativePath = `../../../../assets/cards/${pack.source.folder}/${BACK_IMAGE_FILENAME}`;
            return `  ${JSON.stringify(pack.id)}: require(${JSON.stringify(assetRelativePath)}),`;
          })
          .join("\n");

  const backPacksBlock =
    localBackPacks.length === 0
      ? `
const generatedCardBackPacks: Record<CardBackPackId, ImageSourcePropType> = {};`
      : `
...
export type CardBackPackId = ${cardBackPackIdType};${backPacksBlock}
...
async function main() {
  ...
  ensureBackManifestIntegrity();
  await validateLocalPacks();
  await validateLocalBackPacks();

  const source = generateRegistrySource();
  await fs.mkdir(path.dirname(outputFile), { recursive: true });
  await fs.writeFile(outputFile, source, "utf8");

  const faceCount = CARD_FACE_PACK_MANIFEST.length;
  const backCount = CARD_BACK_PACK_MANIFEST.length;
  console.log(
    `Generated ${path.relative(rootDir, outputFile).replaceAll("\\", "/")} (${faceCount} face, ${backCount} back manifest entries)`,
  );
}
